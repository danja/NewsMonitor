...
public class TestRss2Parser {
  ...
  //	private static Logger log = LoggerFactory.getLogger(TestRss2Parser.class);
  ...
  public void testEntryLevel() {
    ...
    assertEquals(
      "checking entry author",
      "john@doe.com",
      entry0.getAuthor().getEmail()
    );
    assertEquals(
      "checking entry guid",
    ...
    assertEquals(
      "checking entry creator",
      "JohnDoe",
      entry1.getAuthor().getName()
    );
    assertEquals(
      "checking entry2 sort date",
      "2009-09-08T16:20Z",
